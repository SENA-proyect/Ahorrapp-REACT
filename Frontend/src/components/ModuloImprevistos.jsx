                    ...
                    <tbody>
                      {imprevistos.map(i => (
                        <tr key={i.id} className="border-b border-[#D4DCE9] transition-colors hover:bg-slate-50">
                          ...
                          <td className="px-3 py-2.5 align-middle text-sm">{i.causa || '—'}</td>
                          <td className="px-3 py-2.5 align-middle text-sm">{i.categoria || '—'}</td>
                          <td className="px-3 py-2.5 align-middle text-sm">{i.dependiente || '—'}</td>
                          <td className="px-3 py-2.5 align-middle text-sm font-semibold text-[#A72B2F]">${Number(i.monto).toLocaleString('es-CO')}</td>
                          <td className="px-3 py-2.5 align-middle">
                            <div className="flex gap-2">
                              <button onClick={() => abrirEditar(i)} className="rounded-lg border border-purple-500 px-3 py-1 text-xs font-semibold text-purple-700 transition hover:bg-purple-50">Editar</button>
      ...
      {modalEditar && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/45 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-7 shadow-2xl">
            <h4 className="mb-1 text-lg font-bold text-slate-800">Editar Imprevisto</h4>
            ...
            <input className={inputCls} type="text" name="causa" placeholder="Ej: Reparación, Emergencia médica..."
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button onClick={guardarEdicion} disabled={guardando} className="rounded-lg bg-purple-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-purple-700 disabled:bg-purple-300">
      ...
      {confirmarId && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/45 px-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-7 shadow-2xl">
            <h4 className="mb-2 text-lg font-bold text-slate-800">¿Eliminar imprevisto?</h4>
